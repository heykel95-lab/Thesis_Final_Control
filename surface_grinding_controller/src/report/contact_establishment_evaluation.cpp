// ============================================================================
// Contact-establishment evaluation
// ============================================================================
// Evaluating the final contact establishment state, contact wrench, alignment, and compliance
// after a completed contact establishment state.
...
void printSurfaceFrameBreakdown(const ControllerConfig& params,
                                ...
                                const ContactEstablishmentReport& r) {
  ...
  printSection("contact establishment surface-frame breakdown");
...
void reportContactEstablishmentResult(const ControllerConfig& params,
  ...
  // Evaluating the tool-to-plane deviation before and after contact establishment [deg].
  ...
  if (r.t_align >= 0.0) {
    printf("  t_align: %.2f s | tolerance=%.2f deg | hold=%.2f s\n",
           r.t_align, params.contact_establishment_align_tolerance_deg,
           params.contact_establishment_align_hold_time);
  } else {
    printf("  t_align: not reached | tolerance=%.2f deg | hold=%.2f s\n",
           params.contact_establishment_align_tolerance_deg, params.contact_establishment_align_hold_time);
  }
  ...
  if (r.t_align_fraction >= 0.0) {
    printf("  t_align_fraction: %.2f s | fraction=%.2f of the deviation at "
           "contact\n", r.t_align_fraction, params.contact_establishment_align_fraction);
  } else {
    printf("  t_align_fraction: not reached | fraction=%.2f of the deviation "
           "at contact\n", params.contact_establishment_align_fraction);
  }
  ...
  // Evaluating the coupled contact establishment impedance at the final state.
  if (!params.print_compliance_diagnostics ||
      !params.use_virtual_compliance_center) {
    return;
  }

  // Resolving the commanded center of compliance at the final contact establishment pose [m].
  ...
  printf("  %-16s   p_c resolved at end of contact establishment\n", "");
